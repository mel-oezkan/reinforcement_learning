...
A_LR = 0.0003                   # learning rate for actor
...
class PPO:
    ...
    def a_train(self, state, action, adv):
        ...
        adv = tf.convert_to_tensor(adv, np.float32)
    ...
    @tf.function
    def c_train(self, reward, state, new_state):
        """
        ...
        with tf.GradientTape() as tape:
            ...
            # loss = tf.reduce_mean(tf.square(self.critic(state) - advantage))
    ...
    def update(self):
        ...
        n = tf.convert_to_tensor(self.new_state_buffer, np.float32)
