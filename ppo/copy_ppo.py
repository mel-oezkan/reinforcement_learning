...
EP_LEN = 300                    # total number of steps for each episode
...
class Critic(tf.keras.Model):
    def __init__(self):
        ...
        self.fc2 = tf.keras.layers.Dense(100, activation="relu", kernel_regularizer='l2')
...
class PPO:
    ...
    def save_model(sefl, ep):
        path = "model/"

        self.actor_old.save_weights(checkpoint_path.format(epoch=0))
        self.actor.save_weights(checkpoint_path.format(epoch=0))
        self.critic.save_weights(checkpoint_path.format(epoch=0))
